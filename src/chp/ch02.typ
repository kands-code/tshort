...
早期曾出现多种字符编码方案，但是现在基本都被 UTF-8 取代了。
...
连续的空白字符视为一个空格。一行开头的空格忽略不计。

行末的换行符视为一个空格；连续两个换行符，也就是空行，会将文字分段。
多个空行被视为一个空行。也可以在行末使用 `parbreak` 函数分段。
...
Typst *不支持*用符号输入西欧语言中各种拉丁文扩展字符#footnote[
  详情请查看#link("https://github.com/typst/typst/issues/833")[
    typst/issues/833
  ]。
]。
...
虽然平时没有强调，但是在中文排版上的确是有一些标准需要去遵守@w3c-clreq-2025。

在非西文排版方面，Typst 还有待完善#footnote[
  详情请查看#link("https://github.com/typst/typst/issues/193")[
    typst/issues/193
  ]和#link("https://github.com/typst/typst/issues/276")[
    typst/issues/276
  ]。
]。所以，这些标准就需要我们在排版内容的时候自己注意。
这里列举一些比较重要的规则。

=== 行首行尾禁则<ch-3-行首行尾禁则>

为了保持阅读顺畅、体例一致，大多数标点符号的位置有限制，
通常一个标点符号依其性质，禁止出现在行首或行尾。
这项规则自活字排版时代开始通行。

/ 基本处理: 点号（顿号、逗号、句号、冒号、分号、叹号、问号）、结束引号、
  结束括号、结束双书名号、连接号、间隔号、分隔号不能出现在一行的开头。
  开始引号、开始括号、开始单双书名号等符号，不能出现在一行的结尾。

/ GB/T 15834---2011@gbt15834-2011:
  在基本处理的基础上，分隔号 “/” 也不能出现在一行的结尾。

/ 严格处理: 在 GB/T 15834---2011 的基础上，
  破折号、省略号不能出现在一行的开头。
...
由于 @ch-3-行首行尾禁则 等原因，可能造成行的长度长短不一，此时需要进行“行内调整”。
行内调整是指调整可以调整的空间，例如西文词距、标点符号间距等。
...
/ 挤压处理的优先顺序:
  1、位于行末的标点#footnote[
    调成固定的半个汉字宽。
  ]；2、位于行内的西文词距#footnote[
    每个西文词距最小可以挤压到四分之一汉字宽。
  ]；3、位于行内的间隔号；
  4、位于行内的夹注符号；5、位于行内的逗号、顿号、分号；
  6、行内的中西间距；7、位于行内的句号、问号、感叹号。
