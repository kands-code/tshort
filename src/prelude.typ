...
它非常适用于生成高印刷质量的科技类和数学类文档。
它也能够生成所有其他种类的文档 #footnote[
  详情可以参考 `page` 函数的 #link(
...
/ 第一章: 讲述 Typst 的来源，源代码的基本结构，以及如何编译源代码生成文档。
/ 第二章: 讲述 Typst 中如何正确书写文字，包括中文。
/ 第三章: 讲述文档排版的基本元素 --- 标题、目录、列表、图片、表格等等。结合前一章的内容，
  你应当能够制作内容较为丰富的文档了。
/ 第四章: Typst 排版公式的能力不输 LaTeX。本章的内容设计了一些排版公式经常用到的函数，
  章节末尾列出了 Typst 常见的数学符号。
/ 第五章: 介绍了如何修改文档的一些基本样式，包括字体、段落、页面尺寸、页眉页脚等。
/ 第六章: 介绍了 Typst 的一些其他功能：排版参考文献、排版索引、
  排版带有颜色和超链接的电子文档。
/ 第七章: 介绍了如何在 Typst 中使用基础元素绘图，这一部分只是简单介绍。
/ 第八章: 当你相当熟悉前面几章的内容，需要自己编写函数和包（Package）扩展 Typst 的功能时，
  本章介绍了一些基本的语法满足你的需求。
...
要在自己的电脑上安装 Typst，请参考 #ref(<appendix-a>, supplement: "附录") 中的内容。

如果对于本文档内容想要做出调整，请通知作者。
作者对于 Typst 初学者的反馈特别感兴趣，尤其是关于这份文档哪些内容很容易理解，
哪些内容可能需要更好的解释，而那些内容由于太过难以理解、非常不常用而不适宜放在本文档。
